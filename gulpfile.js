...
const srcPath = (file, watch = false) => {
  ...
  if (file === "images")
    return "./website/src/images/**/*.{png,jpeg,jpg,svg,gif}";
  console.error(
    "Unsupported file type entered into Gulp Task Runner for Source Path"
  );
};
const distPath = (file, serve = false) => {
  if (["css", "js", "images"].includes(file)) return `./website/dist/${file}`;
...
const genericTask = (mode, context = "building") => {
  ...
  const browserLoadingWatching = done => {
    ...
    console.log(srcPath("html", true));

    // Watch - Markup
    gulp
      .watch(srcPath("html", true))
      .on(
        "all",
        gulp.series(
          Object.assign(cleanMarkup(mode), {
    ...
    // Watch - Images
    gulp.watch(srcPath("images", true)).on(
      "all",
      gulp.series(
        Object.assign(cleanImages(mode), {
